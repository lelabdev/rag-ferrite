...
fn test_fts5_bm25_search() {
    ...
    // Insert test chunks
    let docs = vec![
        (1, "Rust is a systems programming language focused on safety and speed"),
        (2, "Python is a popular language for machine learning and data science"),
        (3, "The Tokyo metropolitan area is the most populous in the world"),
    ];
    ...
    let results: Vec<(i64, f64)> = stmt.query_map([], |row| {
        Ok((row.get(0)?, -row.get::<_, f64>(1)?)) // negate to make positive
    }).unwrap().filter_map(|r| r.ok()).collect();
    
    assert!(!results.is_empty(), "BM25 should return results for 'Rust programming'");
    assert_eq!(results[0].0, 1, "Chunk about Rust should rank first");
    println!("✅ BM25 search: chunk {} scored {:.6}", results[0].0, results[0].1);
}
...
fn test_cosine_similarity_brute_force() {
    ...
    let mut scored: Vec<(i64, f64)> = chunk_embeddings.iter()
        .map(|(id, emb)| (*id, cosine_similarity(&query_embedding, emb)))
        .collect();
    scored.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    
    assert_eq!(scored[0].0, 1, "Chunk 1 (Rust-like) should have highest cosine similarity");
    assert!(scored[0].1 > 0.9, "Similarity should be high: {:.4}", scored[0].1);
    println!("✅ Cosine similarity: chunk {} scored {:.4}", scored[0].0, scored[0].1);
}
...
fn test_rrf_fusion() {
    ...
    let mut rrf_scores: Vec<(i64, f64)> = vec![1, 2, 3].iter().map(|id| {
        let vec_rank = vector_ranks.get(id).copied();
        let bm25_rank = bm25_ranks.get(id).copied();
        let mut score = 0.0;
        if let Some(r) = vec_rank { score += rrf_score(r, k); }
        if let Some(r) = bm25_rank { score += rrf_score(r, k); }
        (*id, score)
    }).collect();
    ...
    assert!(rrf_scores[0].0 == 1 || rrf_scores[0].0 == 2, "Top result should be chunk 1 or 2");
...
fn test_full_hybrid_pipeline() {
    ...
    let chunks = vec![
        (1, 1, "Rust programming language safety speed concurrency", vec![0.9, 0.1, 0.0]),
        (2, 1, "Rust memory safety borrow checker compile time", vec![0.85, 0.15, 0.0]),
        (3, 2, "Python machine learning data science numpy", vec![0.1, 0.9, 0.0]),
        (4, 2, "Python pandas tensorflow neural networks", vec![0.0, 0.85, 0.1]),
        (5, 3, "Tokyo is the capital of Japan metropolitan area", vec![0.0, 0.0, 0.95]),
    ];
    ...
    let vector_results: Vec<(i64, f64)> = stmt.query_map([], |row| {
        let id: i64 = row.get(0)?;
        let bytes: Vec<u8> = row.get(1)?;
        let emb: Vec<f32> = bytes.chunks_exact(4)
    ...
    assert_eq!(vector_sorted[0].0, 1, "Vector search: chunk 1 should be first");
    assert_eq!(vector_sorted[1].0, 2, "Vector search: chunk 2 should be second");
    ...
    let bm25_results: Vec<(i64, f64)> = stmt.query_map([query_text], |row| {
    ...
    assert!(bm25_chunk_ids.contains(&1) || bm25_chunk_ids.contains(&2), "BM25 should find Rust chunks");
    ...
    let mut all_ids: Vec<i64> = vector_ranks.keys().chain(bm25_ranks.keys()).copied().collect();
    all_ids.sort();
    all_ids.dedup();
    
    let mut rrf_scores: Vec<(i64, f64, u32, u32)> = all_ids.iter().map(|id| {
        let vr = vector_ranks.get(id).copied();
        let br = bm25_ranks.get(id).copied();
        let mut score = 0.0;
        if let Some(r) = vr { score += rrf_score(r, k); }
        if let Some(r) = br { score += rrf_score(r, k); }
        (*id, score, vr.unwrap_or(0) as u32, br.unwrap_or(0) as u32)
    }).collect();
    rrf_scores.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    
    // Top results should be Rust chunks (1 or 2)
    assert!(rrf_scores[0].0 == 1 || rrf_scores[0].0 == 2, 
            "Top result should be a Rust chunk, got chunk {}", rrf_scores[0].0);
    
    println!("✅ Full hybrid pipeline (query: 'Rust programming'):");
    for (id, score, vr, br) in rrf_scores.iter().take(3) {
        println!("   chunk {}: RRF={:.6} (vec_rank={}, bm25_rank={})", id, score, vr, br);
    }
}
...
fn test_fts5_multilingual() {
    ...
    let mut stmt = conn.prepare(
        "SELECT chunk_id FROM chunks_fts WHERE chunks_fts MATCH 'Tokyo' ORDER BY chunk_id"
    ).unwrap();
    let results: Vec<i64> = stmt.query_map([], |row| row.get(0)).unwrap()
        .filter_map(|r| r.ok()).collect();
    
    assert!(results.len() >= 2, "FTS5 should find at least 2 results for 'Tokyo', got {}", results.len());
    println!("✅ FTS5 multilingual: found {} results for 'Tokyo'", results.len());
}
