...
- Retrieval evaluation : `engine::benchmark::run_benchmark` accepts legacy arrays and versioned golden datasets, and reports Recall@k, precision@k, MRR, nDCG, empty-result rate, plus p50/p95 latency. Semantic chunking remains disabled until the reproducible evaluation protocol in `docs/semantic-chunking-evaluation.md` demonstrates a repeatable gain. Persistent per-operation LLM caching remains deferred until operation-level telemetry demonstrates meaningful repeated cost; the protocol is in `docs/llm-cache-evaluation.md`.
