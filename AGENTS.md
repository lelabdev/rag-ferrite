...
- Bounded ingestion queue (mpsc channel + single background worker): REST and MCP share the queue; rebuild/flush operations are serialized with writes, with inline-size, HTTP-body, per-job timeout, and canonical allowed-root path limits. Successful file moves preserve nested paths from `inbox` to its sibling configured ingested directory and atomically refuse destination collisions.
...
- Monitor TUI intégré au binaire (src/monitor/) — dashboard live plus interactive workspaces `1` Dashboard, `2` Library, `3` Query, `4` Ingest, `5` Admin (`j/k` + `d` in Library, `Q` for Query, `i` for Ingest) ; remplace l'ancien binaire standalone rag-monitor
...
- Retrieval evaluation : `engine::benchmark::run_benchmark` accepts legacy arrays and versioned golden datasets, and reports Recall@k, precision@k, MRR, nDCG, empty-result rate, plus p50/p95 latency. Semantic chunking remains disabled until the reproducible evaluation protocol in `docs/semantic-chunking-evaluation.md` demonstrates a repeatable gain. Persistent per-operation LLM caching remains deferred until operation-level telemetry demonstrates meaningful repeated cost; the protocol is in `docs/llm-cache-evaluation.md`. The optional web library console is enabled with `advanced.web_ui_enabled`, served at `/`, and must remain MCP-first, authenticated, and free of chatbot/model functionality. Indexed metadata is rendered through DOM text nodes rather than HTML interpolation. Its scope is documented in `docs/web-interface-plan.md`.
