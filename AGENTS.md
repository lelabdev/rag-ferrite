...
REST/MCP application errors use structured `error_code` values and are mapped to accurate HTTP statuses by `api::json_response`. HTTP defaults to loopback; remote binds require authentication unless the explicit unsafe override is configured, and MCP allowed hosts come from configuration rather than deployment-specific constants.
